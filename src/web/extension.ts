...
async function _retrieveCurrentWeather() {
  let weather = await _loadExternJSONFile("weather.json");
  let readings = Object(weather).main;
  let condition = Object(weather).weather[0];
  return {
    main: condition.main,
    icon: condition.icon,
    temp: (parseInt(readings.temp) - 273.15) * 1.8 + 32,
    feel: (parseInt(readings.feels_like) - 273.15) * 1.8 + 32,
    humid: parseInt(readings.humidity)
  }
}

async function _retrieveCurrentNews() {
...
function getWebViewContent(data: any): string {
  ...
  <html lang="en">
    ...
    <!-- HEADER -->
    <!-- AD SPACE -->
      <img src = '${data.ads.candybar}' id = 'cbAd' style = 'width: 75%; margin: 0 12.5% 5% 12.5%;' class = 'ads'>
    <!-- AD SPACE -->
    <div id = "content" class = "white">
      <div>
        <div id = "term-weather" style="background: #fff; color: #222; overflow: auto;">
          ...
          <div id = "report">
            <div><img src = "https://openweathermap.org/img/wn/${data.weather.icon}@2x.png"></div>
            <div>
              <span class = "temp orange" style = "top: 25%; position:relative;">
                ${parseInt(data.weather.temp)}°
              </span>
            </div>
              <span class = "conditions" style = "vertical-align:top">
                <div>${data.weather.main}</div>
                <div>Humidity: ${parseInt(data.weather.humid)}%</div>
                <div>Feels: ${parseInt(data.weather.feel)}°</div>
              </span>
            </div>
          </div>
        <h2>Live from term-world</h2>
        <img src = "https://unsplash-proxy.glitch.me/random/512x512?query=circuits">
      </div>
    <!-- Column 2 -->
      <div>
        <div>
          <h2 id = "title">${data.news.headline}</h2>
          <h3 id = "byline">${data.news.byline}</h3>
          <p id = "text">
            (${data.news.neighborhood}) ${data.news.text}
          </p>
        </div>
      </div>
      <div>
        <!-- AD SPACE -->
        <img src = '${data.ads.block}' id = "boxAd" class = "ads">
        <!-- AD SPACE -->
    </div>
    </div>
  </html>`;
