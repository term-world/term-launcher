...
export class termPanel {
  ...
  private async _loadAdNetwork() {
    let ads = await fetch(
      "https://raw.githubusercontent.com/term-world/TNN/main/ads/network.json"
    )
    return await ads.json();
  }

  private _getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri): string {
    let ads = (async () => {
      let availableAds = await this._loadAdNetwork();
      return availableAds.json();
    })();
    ...
    <html lang="en">
      <head>
        ...
        <!-- TODO: Revise out jQuery? It's a sux -->
        <script
